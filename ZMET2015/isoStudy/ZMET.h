...
protected: 
	...
	vector<float> *lep_relIso03MR_;
	TBranch *lep_relIso03MR_branch;
	bool lep_relIso03MR_isLoaded;
...
void Init(TTree *tree) {
  ...
  tree->SetMakeClass(1);
	...
	lep_relIso03MR_branch = 0;
	if (tree->GetBranch("lep_relIso03MR") != 0) {
		lep_relIso03MR_branch = tree->GetBranch("lep_relIso03MR");
		if (lep_relIso03MR_branch) {lep_relIso03MR_branch->SetAddress(&lep_relIso03MR_);}
	}
	...
	// load all branches
{
	...
	if (lep_relIso03MR_branch != 0) lep_relIso03MR();
	...
	const vector<float> &lep_relIso03MR()
	{
		if (not lep_relIso03MR_isLoaded) {
			if (lep_relIso03MR_branch != 0) {
				lep_relIso03MR_branch->GetEntry(index);
			} else { 
				printf("branch lep_relIso03MR_branch does not exist!\n");
				exit(1);
			}
			lep_relIso03MR_isLoaded = true;
		}
		return *lep_relIso03MR_;
	}
