...
void plothtandpt()
{
  ...
  h_zpt_2j->GetYaxis()->SetRangeUser(2e-7,1);
  h_zpt_3j->GetYaxis()->SetRangeUser(2e-7,1);
  ...
  l1->AddEntry( h_zpt_2j , Form("%s_{T}(Z): 2 Jets"      , variable_tag.c_str() ), "le");
  l1->AddEntry( h_zpt_3j , Form("%s_{T}(Z): 3 Jets"      , variable_tag.c_str() ), "le");
  l1->AddEntry( h_gpt_2j , Form("%s_{T}(#gamma): 2 Jets" , variable_tag.c_str() ), "le");
  l1->AddEntry( h_gpt_3j , Form("%s_{T}(#gamma): 3 Jets" , variable_tag.c_str() ), "le");

  l1->Draw("same");
 

  c1->SaveAs(Form("../output/ZMET2015/V00-00-20/plots/Closure/h_%st_2vs3jets.png", variable_tag.c_str() ));
  c1->SaveAs(Form("../output/ZMET2015/V00-00-20/plots/Closure/h_%st_2vs3jets.pdf", variable_tag.c_str() ));
  ...
  TLegend *l2 = new TLegend(0.6, 0.6, 0.9, 0.90);    
  l2->SetLineColor(kWhite);    
  l2->SetShadowColor(kWhite);    
  l2->SetFillColor(kWhite);    
  l2->AddEntry( h_zpt_2j , Form("%s_{T}(Z): 2 Jets"      , variable_tag.c_str() ), "le");
  l2->AddEntry( h_gpt_2j , Form("%s_{T}(#gamma): 2 Jets" , variable_tag.c_str() ), "le");

  photon_htcuts.clear();
  photon_htcuts.push_back(50);
  photon_htcuts.push_back(75);
  photon_htcuts.push_back(90);
  photon_htcuts.push_back(135);
  ...
  c1->SaveAs(Form("../output/ZMET2015/V00-00-20/plots/Closure/h_%st_2jets_newbins.png", variable_tag.c_str() ));
  c1->SaveAs(Form("../output/ZMET2015/V00-00-20/plots/Closure/h_%st_2jets_newbins.pdf", variable_tag.c_str() ));



  // h_zpt_2j->Draw("histe1");
  h_zpt_3j->Draw("histe1");
  // h_gpt_2j->Draw("histe1same");
  h_gpt_3j->Draw("histe1same");
  ...
  TLegend *l3 = new TLegend(0.6, 0.6, 0.9, 0.90);    
  l3->SetLineColor(kWhite);    
  l3->SetShadowColor(kWhite);    
  l3->SetFillColor(kWhite);    
  l3->AddEntry( h_zpt_3j , Form("%s_{T}(Z): 3 Jets"      , variable_tag.c_str() ), "le");
  l3->AddEntry( h_gpt_3j , Form("%s_{T}(#gamma): 3 Jets" , variable_tag.c_str() ), "le");
  ...
  c1->SaveAs(Form("../output/ZMET2015/V00-00-20/plots/Closure/h_%st_3jets_newbins.png", variable_tag.c_str() ));
  c1->SaveAs(Form("../output/ZMET2015/V00-00-20/plots/Closure/h_%st_3jets_newbins.pdf", variable_tag.c_str() ));
